...
# make tertiles of parental warmth
make_derived_vars = function(dat) {

  # should we keep the subject in the supplemental analysis of top vs. bottom tertile?
  dat$keep.supp = ( dat$A1SEPA_z < quantile(dat$A1SEPA_z, 1/3, na.rm = TRUE) ) |
    ( dat$A1SEPA_z > quantile(dat$A1SEPA_z, 2/3, na.rm = TRUE ) )
  
  # don't include anyone who is missing A1SEPA_z
  dat$keep.supp[ is.na(dat$keep.supp) ] = 0
  
  # indicator for warmth tertile
  # equals 1 if in top tertile
  # 0 if in bottom tertile 
  # NA otherwise so that observation will be dropped from analysis
  dat$A1SEPA_top = NA
  dat$A1SEPA_top[ !is.na(dat$A1SEPA_z) & dat$A1SEPA_z > quantile(dat$A1SEPA_z, 2/3, na.rm = TRUE) ] = 1
  dat$A1SEPA_top[ !is.na(dat$A1SEPA_z) & dat$A1SEPA_z < quantile(dat$A1SEPA_z, 1/3, na.rm = TRUE) ] = 0
